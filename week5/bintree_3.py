# Adding breadthfirst method to the bintree_2.py     
...
class BST:
    ...
    # prints out the content of the search tree in breath-first order
    def breadthfirst(self):
        nodeArray = [self.root]         

        while(len(nodeArray) != 0): 
            node = nodeArray.pop(0)     # pop in this case deletes the old node from the array

            if (node.left != None):             # checks if there's left child
                nodeArray.append(node.left)

            if (node.right != None):            # checks if there's right child
